...
**My single source of truth for this project.** Every phase below states what I do, why I do it, what it creates, and the concrete deliverable. I keep this document versioned at `docs/PROJECT_BLUEPRINT.md` and update it in place as my decisions evolve — I treat changes to it like an ADR (architecture decision record) log, not a static plan. v2 supersedes v1 after I ran two independent technical reviews on my own work; §0.2 records what I changed and why, so the reasoning isn't lost.

Author: Henry Otsyula. Competition: Climate & Health Risk Prediction Challenge. Opened 18 Aug 2026, closes 19 Oct 2026 (public leaderboard is a partial-test-set score during the competition; the info page states the full private evaluation is revealed at close — confirmed from the competition's own leaderboard page text, not assumed). Prize: 1st place $500.

## 0.1 Verified data forensics (run before writing this revision, not assumed)

My v1 stated some structural claims about the data as fact without my having executed the code to confirm them. Both reviews I ran flagged this as the single most important gap — most sharply Review 2 point 1, which named it a blocking dependency on the whole validation strategy — so before revising anything else, I actually ran the checks against the raw files. Results, which now anchor every downstream decision I make in this document:
...
I read both reviews in full and checked them against the actual data wherever they made a checkable claim (§0.1 above covers the empirical ones). What follows is my disposition of every substantive point, so my reasoning is auditable later rather than silently baked in.
...
I'm pursuing two objectives with different deadlines — win the competition (hard deadline 19 Oct 2026) and produce a senior-level portfolio piece (no hard deadline) — sharing one repository and one engineered core, rather than treating the portfolio as a layer I bolt on after the fact:
...
Stage 1 (repo scaffold) is still the right starting point — unchanged from v1, since everything above depends on it existing. What changes is what fills the scaffold next: Stage 3 (data forensics notebook) is now explicitly first, seeded directly from §0.1's already-verified findings, before general EDA — the reviews' strongest shared point was that I need to know how the data was put together before I start modeling it, and I've already started that work in this document.
